...
import { ReporterRuntime, createDefaultWriter, getEnvironmentLabels, getFallbackTestCaseIdLabel, getFrameworkLabel, getHostLabel, getLanguageLabel, getPackageLabel, getPosixPath, getProjectName, getSuiteLabels, getThreadLabel, includedInTestPlan, md5, parseTestPlan } from "allure-js-commons/sdk/reporter";
...
var createJestEnvironment = Base => {
  ...
  return _Class_brand = /*#__PURE__*/new WeakSet(), class extends Base {
    ...
    constructor(config, context) {
      ...
      var handleTestEvent = (_this$handleTestEvent = this.handleTestEvent) === null || _this$handleTestEvent === void 0 ? void 0 : _this$handleTestEvent.bind(this);

      // Preserve any event handler defined by the base or custom environment.
      this.handleTestEvent = (event, state) => {
        var handleAllureEvent = () => _assertClassBrand(_Class_brand, this, _handleTestEvent).call(this, event, state);
        ...
        try {
          var result = handleTestEvent === null || handleTestEvent === void 0 ? void 0 : handleTestEvent(event, state);
          if (isPromise(result)) {
  ...
  function _handleTestEvent(event, _state) {
    switch (event.name) {
      case "hook_start":
  ...
  function _getTestFullName(test) {
    ...
    var fullNamePath = getPosixPath(this.testPath);
    var fullNameBase = projectName ? "".concat(projectName, ":").concat(fullNamePath) : fullNamePath;
    return "".concat(fullNameBase, "#").concat(newTestId);
  }
  ...
  function _handleTestStart(test) {
    ...
    var newTestSuitePath = getTestPath(test.parent);
    var projectName = getProjectName();
    var baseTitlePath = fsPath.concat(newTestSuitePath);
    var titlePath = projectName ? [projectName, ...baseTitlePath] : baseTitlePath;
    var {
      cleanTitle,
      labels,
      links
    } = extractMetadataFromString(test.name);
    var legacyTestId = getTestId(newTestSuitePath.concat(cleanTitle));
    var legacyFullName = "".concat(getPosixPath(this.testPath), "#").concat(legacyTestId);
    var newTestFullName = _assertClassBrand(_Class_brand, this, _getTestFullName).call(this, test, cleanTitle);
    if (this.testPlan && !includedInTestPlan(this.testPlan, {
    ...
    var testUuid = this.runtime.startTest({
      ...
      stage: Stage.RUNNING,
      labels: [getLanguageLabel(), getFrameworkLabel("jest"), getPackageLabel(this.testPath), getFallbackTestCaseIdLabel(md5(legacyFullName)), getHostLabel(), getThreadLabel(env.JEST_WORKER_ID), ...getEnvironmentLabels(), ...getSuiteLabels(newTestSuitePath), ...labels],
      titlePath,
      links
